...
if [ -z "$found" ]; then
  ...
  if [ "$spi_select" == "carrier" ]; then
  	echo CARRIER_SPI_ID=FAIL >> CARRIER_QSEVEN_BIOS.txt
  else [ "$spi_select" == "qseven" ];
  	echo QSEVEN_SPI_ID=FAIL >> CARRIER_QSEVEN_BIOS.txt	
  fi
  exit 1
else
...
#upgrade SPI
  ...
  if [ "$spi_select" == "carrier" ]; then
    ...
    . /usr/lib/gizmo-common/ami-firmware-common.sh
    afulnx /home/root/$2 /R /B /P /N /X
