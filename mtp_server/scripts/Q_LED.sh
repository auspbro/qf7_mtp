...
usage()
{
echo "Enter current 0xFF R/G/B/O/W 0x64"
echo "R Red"
echo "G Green"
echo "B Blue"
echo "O Off"
    ...
    for data in a0 a1 a2 a3 a4 a5 a6 a7 a8; do
      ...
      if [ "$v_mv" -lt "$vf_limit_high" ]; then
        if [ "$v_mv" -gt "$vf_limit_low" ]; then
          ...
          echo VF_LOW-$bank-$i2c_add-$data=PASS >> Q_LED_DETECT_$4.txt
	 else
          echo "FAIL, LED Vf too low, Bank $bank, Dev 0x$i2c_add, LED $data, possible short circuit"
          echo VF_LOW_$bank-$i2c_add-$data=FAIL >> Q_LED_DETECT_$4.txt
        fi
      else
        echo "FAIL, LED Vf too high, Bank $bank, Dev 0x$i2c_add, LED $data, possible open circuit"
        echo VF_HIGH-$bank-$i2c_add-$data=PASS >> Q_LED_DETECT_$4.txt
	fi
    done
  done
done
...
cp Q_LCD_DETECT.txt /run/media/sdb1/
elif [ "$color" == "DCP" ]; then
