...
if [ "$v_mil" -lt "$v_limit_high" ]; then
  if [ "$v_mil" -gt "$v_limit_low" ]; then
    ...
    echo CARRIER_HEALTH_R_VOG=$v_mil >> Q_CARRIER_HEALTH.txt
  else
...
if [ "$v_mil" -lt "$i_limit_high" ]; then
  if [ "$v_mil" -gt "$i_limit_low" ]; then
    ...
    echo CARRIER_HEALTH_R_CUR=$v_mil >> Q_CARRIER_HEALTH.txt
  else
...
if [ "$v_mil" -lt "$p_limit_high" ]; then
  if [ "$v_mil" -gt "$p_limit_low" ]; then
    ...
    echo CARRIER_HEALTH_R_PWR=$v_mil >> Q_CARRIER_HEALTH.txt
  else
  ...
  if [ "$t_dec" -lt "$t_limit_high" ]; then
    if [ "$t_dec" -gt "$t_limit_low" ]; then
      echo "PASS, TMP108 at 0x$i2c_add IN spec degC, $t_dec"
         if [ "$i2c_add" == "48" ]; then
    			echo CARRIER_HEALTH_TMP108_48=$t_dec >> Q_CARRIER_HEALTH.txt
    			elif [ "$i2c_add" == "4a" ]; then
    			echo CARRIER_HEALTH_TMP108_4a=$t_dec >> Q_CARRIER_HEALTH.txt
    			else
      		usage
    		 fi
    else
